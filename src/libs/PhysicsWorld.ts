...
export class PhysicsWorld {
      ...
      if (!(ballState[n_offset + 24] & 5)) {
        ...
        if (maxThreshold > 0) {
          ...
          if (dist > maxThreshold) {
            ...
            // Progressive rubber-band return acceleration (bounded to avoid extreme forces)
            ...
            // Multiplicative damping beyond threshold (softly decelerates outward motion)
  ...
  _cachePanelCollisions(
    ...
    for (let s_panel = 0; s_panel < r_panels; s_panel++) {
      ...
      let d_val = input[o_idx + 5];
      let f_val = input[o_idx + 6];
      let p_val = input[o_idx + 7];
